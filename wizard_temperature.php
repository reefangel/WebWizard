...
$( function() {
	$('#prev').button({
	  icon: "ui-icon-triangle-1-w"
		...
		localStorage.setItem("temperatureunit", $('input:checked').val());
	...
	$( "input[name=tempsettings]" ).checkboxradio();
	if(localStorage.getItem("temperatureunit")!="")
	{
		$("input[name=tempsettings][value=" + localStorage.getItem("temperatureunit") + "]").prop('checked', true);
		$("input[name=tempsettings]").checkboxradio( "refresh" );
	}
});
</script>
...
<div id="main" class="split split-horizontal">
	...
	<p>How would you like to see your temperature readings?</p>
    <label for="celsius">Celsius</label>
    <input type="radio" id="celsius" name="tempsettings" value="celsius">
    <br>
    <label for="fahrenheit">Fahrenheit</label>
    <input type="radio" id="fahrenheit" name="tempsettings" value="fahrenheit" checked>
    <div id=footer>
